...
const ARROW_CLASSES =
  "relative flex items-center justify-center rounded-full border-[1.5px] border-[#A97142]/30 bg-[#F8F5EF]/80 text-[#332828]/60 cursor-pointer shrink-0 z-30 outline-none shadow-[0_4px_20px_rgba(169,113,66,0.15)] hover:border-[#A97142]/60 hover:text-[#332828] active:opacity-70 transition-colors duration-300 before:content-[''] before:absolute before:inset-[3px] before:rounded-full before:border before:border-[#A97142]/10 before:pointer-events-none";
      ...
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem] h-[400px] md:h-[500px]">
          {cards.map((card, index) => {
            const isPdf = card.imgUrl.toLowerCase().endsWith('.pdf');
            const image = (
              <div className="relative w-full h-full overflow-hidden bg-white">
                {isPdf ? (
                  <iframe 
                    src={`${card.imgUrl}#toolbar=0&navpanes=0&scrollbar=0`} 
                    title={`Certificate ${index}`} 
                    className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[110%] h-[110%] object-cover pointer-events-none border-none" 
                  />
                ) : (
                  <img src={card.imgUrl} loading="lazy" alt={card.alt || `Card ${index}`} className="absolute inset-0 w-full h-full object-cover pointer-events-none" />
                )}
            ...
            const cardClasses = "fan-card absolute w-[260px] h-[190px] sm:w-[320px] sm:h-[235px] md:w-[480px] md:h-[350px] lg:w-[600px] lg:h-[440px] rounded-xl md:rounded-2xl shadow-xl overflow-hidden cursor-pointer border border-[#A97142]/20";
            return card.linkUrl ? (
              <a key={index} href={card.linkUrl} target={card.linkUrl.startsWith("http") ? "_blank" : "_self"} rel="noopener noreferrer" className={cardClasses}>{image}</a>
            ) : (
      ...
      {needsPagination && (
        <div className="flex items-center justify-center gap-4 mt-4 md:mt-6 z-30">
          ...
          <div className="flex items-center gap-2">
            {cards.map((_, i) => (
              <span key={i} className={`w-2 h-2 rounded-full transition-all duration-300 ${i === centerIndex ? "bg-[#A97142] scale-[1.3]" : "bg-[#A97142]/30"}`} />
            ))}
          </div>
